...
const categoryGradients: Record<string, string> = {
  Electronics: 'from-navy-400 to-navy-700',
  Clothing: 'from-primary-400 to-primary-700',
  Books: 'from-navy-300 to-primary-600',
};
      ...
      <Card className="group h-full overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
        ...
        <div className={`relative aspect-square bg-gradient-to-br ${gradient} flex items-center justify-center overflow-hidden`}>
          ...
          <div className="absolute top-3 left-3">
            <Badge variant="secondary" className="bg-white/90 text-navy-700">
              {product.category}
            </Badge>
          </div>
        ...
        <CardContent className="p-4">
          <h3 className="font-semibold text-navy-950 group-hover:text-primary-500 transition-colors line-clamp-1">
            {product.name}
          </h3>
          <p className="mt-1 text-sm text-navy-500 line-clamp-2">{product.description}</p>

          <div className="mt-3 flex items-center justify-between">
            <span className="text-lg font-bold text-navy-950">{formatPrice(product.price)}</span>
            <Button
              size="sm"
